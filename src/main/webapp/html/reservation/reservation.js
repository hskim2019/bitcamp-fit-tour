 ...
 * https://github.com/Kinark/Materialize-stepper // JS v2.1.3
 */
...
$.fn.nextStep = function(callback, activefb, e) {
   ...
   // If the stepper is parallel, we want to validate the input of the current
    // active step. Not all elements.
...
$.fn.activateStepper = function(options) {
   ...
   $(this).each(function(){
      ...
      $stepper.on("click", '.step:not(.active)', function () {
         ...
         if($stepper.data('settings').parallel && validation) { // Invoke
                                                                // parallel
                                                                // stepper
                                                                // behaviour
...
/**
 * Run validation over all previous steps from the steps this function is called
 * on.
 */
...
$.fn.validateStep = function() {
    ...
    if(this.validateStepInput()) { // If initial base validation succeeded go
                                    // on
...
/**
 * Uses the validation variable set by the stepper constructor to run standard
 * validation on the current step.
 * 
 * @returns {boolean}
 */
