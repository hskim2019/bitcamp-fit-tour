...
-- 상품
DROP TABLE IF EXISTS `MY_SCHEMA`.`tour` RESTRICT;

-- 일정(후기)
DROP TABLE IF EXISTS `MY_SCHEMA`.`template_review_date` RESTRICT;

-- 여행후기(템플릿)
DROP TABLE IF EXISTS `MY_SCHEMA`.`template_review` RESTRICT;

-- 현지투어상품
DROP TABLE IF EXISTS `MY_SCHEMA`.`local_tour_template_review` RESTRICT;

-- 레스토랑
DROP TABLE IF EXISTS `MY_SCHEMA`.`restaurant_review` RESTRICT;

-- 숙소
DROP TABLE IF EXISTS `MY_SCHEMA`.`accommodation_review` RESTRICT;

-- 관광지
DROP TABLE IF EXISTS `MY_SCHEMA`.`tourist_attraction_review` RESTRICT;

-- 교통
DROP TABLE IF EXISTS `MY_SCHEMA`.`transportation_review` RESTRICT;

-- 자유후기
DROP TABLE IF EXISTS `MY_SCHEMA`.`free_review` RESTRICT;

-- 상품안내사진
DROP TABLE IF EXISTS `MY_SCHEMA`.`tour_guidance_photo` RESTRICT;

-- 예약
DROP TABLE IF EXISTS `MY_SCHEMA`.`reservation` RESTRICT;

-- 1:1 문의 
DROP TABLE IF EXISTS `MY_SCHEMA`.`personal_inquiry` RESTRICT;

-- 상품댓글
DROP TABLE IF EXISTS `MY_SCHEMA`.`tour_comment` RESTRICT;

-- 공지사항
DROP TABLE IF EXISTS `MY_SCHEMA`.`notice` RESTRICT;

-- 판매불가 날짜
DROP TABLE IF EXISTS `MY_SCHEMA`.`imposibility_date` RESTRICT;

-- 코스
DROP TABLE IF EXISTS `MY_SCHEMA`.`tour_course` RESTRICT;

-- 코스사진
DROP TABLE IF EXISTS `MY_SCHEMA`.`tour_course_photo` RESTRICT;

-- 여행테마
DROP TABLE IF EXISTS `MY_SCHEMA`.`theme` RESTRICT;

-- 상품여행테마
DROP TABLE IF EXISTS `MY_SCHEMA`.`tour_theme` RESTRICT;

-- 상품회원(위시리스트)
DROP TABLE IF EXISTS `MY_SCHEMA`.`member_tour_wishlist` RESTRICT;

-- 후기회원(좋아요)
DROP TABLE IF EXISTS `MY_SCHEMA`.`member_tour_template_review_like` RESTRICT;

-- 자유후기(좋아요)
DROP TABLE IF EXISTS `MY_SCHEMA`.`member_tour_free_review_like` RESTRICT;

-- 현지투어(자유후기)
DROP TABLE IF EXISTS `MY_SCHEMA`.`local_tour_free_review` RESTRICT;

-- 현지투어(자유후기)사진
DROP TABLE IF EXISTS `MY_SCHEMA`.`local_tour_free_review_photo` RESTRICT;

-- 템플릿후기사진
DROP TABLE IF EXISTS `MY_SCHEMA`.`template_review_photo` RESTRICT;

-- 자유후기 사진
DROP TABLE IF EXISTS `MY_SCHEMA`.`free_review_photo` RESTRICT;

-- 도시
DROP TABLE IF EXISTS `MY_SCHEMA`.`tour_free_review_city` RESTRICT;

-- 국가
DROP TABLE IF EXISTS `MY_SCHEMA`.`tour_free_review_country` RESTRICT;

-- 자유후기댓글
DROP TABLE IF EXISTS `MY_SCHEMA`.`free_review_comment` RESTRICT;

-- 템플릿여행후기댓글
DROP TABLE IF EXISTS `MY_SCHEMA`.`template_review_comment` RESTRICT;

-- 로그인유형
DROP TABLE IF EXISTS `MY_SCHEMA`.`login_type` RESTRICT;

-- 여행활동
DROP TABLE IF EXISTS `MY_SCHEMA`.`travel_activity` RESTRICT;

-- 여행활동유형
DROP TABLE IF EXISTS `MY_SCHEMA`.`activity_type` RESTRICT;

-- 자유후기방문도시
DROP TABLE IF EXISTS `MY_SCHEMA`.`free_review_city` RESTRICT;

-- 결제상태
DROP TABLE IF EXISTS `MY_SCHEMA`.`payment_status` RESTRICT;
...
-- 일정(후기)
CREATE TABLE `MY_SCHEMA`.`template_review_date` (
...
-- 일정(후기)
ALTER TABLE `MY_SCHEMA`.`template_review_date`
...
-- 여행후기(템플릿)
CREATE TABLE `MY_SCHEMA`.`template_review` (
`review_id`      INTEGER      NOT NULL COMMENT '여행후기 번호', -- 여행후기 번호
`member_id`      INTEGER      NOT NULL COMMENT '회원번호', -- 회원번호
...
-- 여행후기(템플릿)
ALTER TABLE `MY_SCHEMA`.`template_review`
...
-- 현지투어상품
CREATE TABLE `MY_SCHEMA`.`local_tour_template_review` (
...
-- 현지투어상품
ALTER TABLE `MY_SCHEMA`.`local_tour_template_review`
...
-- 레스토랑
CREATE TABLE `MY_SCHEMA`.`restaurant_review` (
`activity_id` INTEGER     NOT NULL COMMENT '여행활동번호', -- 여행활동번호
...
-- 레스토랑
ALTER TABLE `MY_SCHEMA`.`restaurant_review`
...
-- 숙소
CREATE TABLE `MY_SCHEMA`.`accommodation_review` (
`activity_id`     INTEGER NOT NULL COMMENT '여행활동번호', -- 여행활동번호
`rating`          INTEGER NOT NULL COMMENT '평점', -- 평점
...
-- 숙소
ALTER TABLE `MY_SCHEMA`.`accommodation_review`
...
-- 관광지
CREATE TABLE `MY_SCHEMA`.`tourist_attraction_review` (
`activity_id` INTEGER NOT NULL COMMENT '여행활동번호', -- 여행활동번호
...
-- 관광지
ALTER TABLE `MY_SCHEMA`.`tourist_attraction_review`
...
-- 교통
CREATE TABLE `MY_SCHEMA`.`transportation_review` (
`activity_id`   INTEGER     NOT NULL COMMENT '여행활동번호', -- 여행활동번호
...
-- 교통
ALTER TABLE `MY_SCHEMA`.`transportation_review`
...
-- 자유후기
CREATE TABLE `MY_SCHEMA`.`free_review` (
...
-- 자유후기
ALTER TABLE `MY_SCHEMA`.`free_review`
...
-- 상품안내사진
CREATE TABLE `MY_SCHEMA`.`tour_guidance_photo` (
...
-- 상품안내사진
ALTER TABLE `MY_SCHEMA`.`tour_guidance_photo`
...
-- 1:1 문의 
CREATE TABLE `MY_SCHEMA`.`personal_inquiry` (
...
-- 1:1 문의 
ALTER TABLE `MY_SCHEMA`.`personal_inquiry`
...
-- 상품댓글
CREATE TABLE `MY_SCHEMA`.`tour_comment` (
...
-- 상품댓글
ALTER TABLE `MY_SCHEMA`.`tour_comment`
...
-- 판매불가 날짜
CREATE TABLE `MY_SCHEMA`.`imposibility_date` (
...
-- 판매불가 날짜
ALTER TABLE `MY_SCHEMA`.`imposibility_date`
...
-- 코스
CREATE TABLE `MY_SCHEMA`.`tour_course` (
...
-- 코스
ALTER TABLE `MY_SCHEMA`.`tour_course`
...
-- 코스사진
CREATE TABLE `MY_SCHEMA`.`tour_course_photo` (
`photo_id`   INTEGER      NOT NULL COMMENT '사진번호', -- 사진번호
`course_id`  INTEGER      NOT NULL COMMENT '코스번호', -- 코스번호
`photo_name` VARCHAR(255) NOT NULL COMMENT '사진명', -- 사진명
`photo_path` VARCHAR(255) NOT NULL COMMENT '사진 경로' -- 사진 경로
...
-- 코스사진
ALTER TABLE `MY_SCHEMA`.`tour_course_photo`
...
-- 상품여행테마
CREATE TABLE `MY_SCHEMA`.`tour_theme` (
`tour_id`  INTEGER NOT NULL COMMENT '상품번호', -- 상품번호
`theme_id` INTEGER NOT NULL COMMENT '여행테마번호' -- 여행테마번호
)
COMMENT '상품여행테마';
...
-- 상품여행테마
ALTER TABLE `MY_SCHEMA`.`tour_theme`
...
-- 상품회원(위시리스트)
CREATE TABLE `MY_SCHEMA`.`member_tour_wishlist` (
`member_id` INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
`tour_id`   INTEGER NOT NULL COMMENT '상품번호' -- 상품번호
)
COMMENT '상품회원(위시리스트)';
...
-- 상품회원(위시리스트)
ALTER TABLE `MY_SCHEMA`.`member_tour_wishlist`
...
-- 후기회원(좋아요)
CREATE TABLE `MY_SCHEMA`.`member_tour_template_review_like` (
`review_id` INTEGER NOT NULL COMMENT '여행후기 번호', -- 여행후기 번호
`member_id` INTEGER NOT NULL COMMENT '회원번호' -- 회원번호
)
COMMENT '후기회원(좋아요)';
...
-- 후기회원(좋아요)
ALTER TABLE `MY_SCHEMA`.`member_tour_template_review_like`
...
-- 자유후기(좋아요)
CREATE TABLE `MY_SCHEMA`.`member_tour_free_review_like` (
`free_review_id` INTEGER NOT NULL COMMENT '자유후기번호', -- 자유후기번호
`member_id`      INTEGER NOT NULL COMMENT '회원번호' -- 회원번호
)
COMMENT '자유후기(좋아요)';
...
-- 자유후기(좋아요)
ALTER TABLE `MY_SCHEMA`.`member_tour_free_review_like`
...
-- 현지투어(자유후기)
CREATE TABLE `MY_SCHEMA`.`local_tour_free_review` (
...
-- 현지투어(자유후기)
ALTER TABLE `MY_SCHEMA`.`local_tour_free_review`
...
-- 현지투어(자유후기)사진
CREATE TABLE `MY_SCHEMA`.`local_tour_free_review_photo` (
`photo_id`   INTEGER      NOT NULL COMMENT '사진번호', -- 사진번호
`review_id`  INTEGER      NOT NULL COMMENT '현지투어번호', -- 현지투어번호
`photo_name` VARCHAR(255) NOT NULL COMMENT '사진명', -- 사진명
`photo_path` VARCHAR(255) NOT NULL COMMENT '사진 경로' -- 사진 경로
...
-- 현지투어(자유후기)사진
ALTER TABLE `MY_SCHEMA`.`local_tour_free_review_photo`
...
-- 템플릿후기사진
CREATE TABLE `MY_SCHEMA`.`template_review_photo` (
`photo_id`    INTEGER      NOT NULL COMMENT '사진번호', -- 사진번호
`activity_id` INTEGER      NOT NULL COMMENT '여행활동번호', -- 여행활동번호
`photo_name`  VARCHAR(255) NOT NULL COMMENT '사진명', -- 사진명
`photo_path`  VARCHAR(255) NOT NULL COMMENT '사진 경로' -- 사진 경로
...
-- 템플릿후기사진
ALTER TABLE `MY_SCHEMA`.`template_review_photo`
...
-- 자유후기 사진
CREATE TABLE `MY_SCHEMA`.`free_review_photo` (
`photo_id`       INTEGER      NOT NULL COMMENT '사진번호', -- 사진번호
`free_review_id` INTEGER      NOT NULL COMMENT '자유후기번호', -- 자유후기번호
`photo_name`     VARCHAR(255) NOT NULL COMMENT '사진명', -- 사진명
`photo_path`     VARCHAR(255) NOT NULL COMMENT '사진 경로' -- 사진 경로
...
-- 자유후기 사진
ALTER TABLE `MY_SCHEMA`.`free_review_photo`
...
-- 도시
CREATE TABLE `MY_SCHEMA`.`tour_free_review_city` (
`city_id`    INTEGER     NOT NULL COMMENT '도시번호', -- 도시번호
...
-- 도시
ALTER TABLE `MY_SCHEMA`.`tour_free_review_city`
...
-- 국가
CREATE TABLE `MY_SCHEMA`.`tour_free_review_country` (
`country_id` INTEGER     NOT NULL COMMENT '국가번호', -- 국가번호
...
-- 국가
ALTER TABLE `MY_SCHEMA`.`tour_free_review_country`
...
ON `MY_SCHEMA`.`tour_free_review_country` ( -- 국가
`country` ASC -- 국가
);
...
-- 자유후기댓글
CREATE TABLE `MY_SCHEMA`.`free_review_comment` (
...
-- 자유후기댓글
ALTER TABLE `MY_SCHEMA`.`free_review_comment`
...
-- 템플릿여행후기댓글
CREATE TABLE `MY_SCHEMA`.`template_review_comment` (
...
-- 템플릿여행후기댓글
ALTER TABLE `MY_SCHEMA`.`template_review_comment`
...
-- 로그인유형
CREATE TABLE `MY_SCHEMA`.`login_type` (
`login_type_id` INTEGER     NOT NULL COMMENT '로그인유형번호', -- 로그인유형번호
`type`          VARCHAR(50) NOT NULL COMMENT '로그인유형' -- 로그인유형
)
COMMENT '로그인유형';

-- 로그인유형
ALTER TABLE `MY_SCHEMA`.`login_type`
...
-- 여행활동
CREATE TABLE `MY_SCHEMA`.`travel_activity` (
`activity_id` INTEGER      NOT NULL COMMENT '여행활동번호', -- 여행활동번호
`date_id`     INTEGER      NOT NULL COMMENT '일정번호', -- 일정번호
...
-- 여행활동
ALTER TABLE `MY_SCHEMA`.`travel_activity`
...
-- 여행활동유형
CREATE TABLE `MY_SCHEMA`.`activity_type` (
`type_id` INTEGER     NOT NULL COMMENT '여행활동유형번호', -- 여행활동유형번호
...
-- 여행활동유형
ALTER TABLE `MY_SCHEMA`.`activity_type`
...
ON `MY_SCHEMA`.`activity_type` ( -- 여행활동유형
`type` ASC -- 유형명
);
...
-- 자유후기방문도시
CREATE TABLE `MY_SCHEMA`.`free_review_city` (
`free_review_id` INTEGER NOT NULL COMMENT '자유후기번호', -- 자유후기번호
`city_id`        INTEGER NOT NULL COMMENT '도시번호' -- 도시번호
)
COMMENT '자유후기방문도시';
...
-- 자유후기방문도시
ALTER TABLE `MY_SCHEMA`.`free_review_city`
...
-- 결제상태
CREATE TABLE `MY_SCHEMA`.`payment_status` (
`status_id` INTEGER     NOT NULL COMMENT '상태번호', -- 상태번호
...
-- 결제상태
ALTER TABLE `MY_SCHEMA`.`payment_status`
...
ON `MY_SCHEMA`.`payment_status` ( -- 결제상태
`status` ASC -- 결제상태
);
...
REFERENCES `MY_SCHEMA`.`login_type` ( -- 로그인유형
`login_type_id` -- 로그인유형번호
);
...
REFERENCES `MY_SCHEMA`.`template_review` ( -- 여행후기(템플릿)
`review_id` -- 여행후기 번호
);
...
REFERENCES `MY_SCHEMA`.`tour_free_review_city` ( -- 도시
`city_id` -- 도시번호
);
...
REFERENCES `MY_SCHEMA`.`travel_activity` ( -- 여행활동
`activity_id` -- 여행활동번호
);
...
REFERENCES `MY_SCHEMA`.`payment_status` ( -- 결제상태
`status_id` -- 상태번호
);
...
REFERENCES `MY_SCHEMA`.`tour_course` ( -- 코스
`course_id` -- 코스번호
);
...
REFERENCES `MY_SCHEMA`.`theme` ( -- 여행테마
`theme_id` -- 여행테마번호
);
...
REFERENCES `MY_SCHEMA`.`free_review` ( -- 자유후기
`free_review_id` -- 자유후기번호
);
...
REFERENCES `MY_SCHEMA`.`local_tour_free_review` ( -- 현지투어(자유후기)
`review_id` -- 현지투어번호
);
...
REFERENCES `MY_SCHEMA`.`tour_free_review_country` ( -- 국가
`country_id` -- 국가번호
);
...
REFERENCES `MY_SCHEMA`.`activity_type` ( -- 여행활동유형
`type_id` -- 여행활동유형번호
);
...
REFERENCES `MY_SCHEMA`.`template_review_date` ( -- 일정(후기)
`date_id` -- 일정번호
);
